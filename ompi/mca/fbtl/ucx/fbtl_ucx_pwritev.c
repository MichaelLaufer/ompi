...
static ssize_t mca_fbtl_ucx_pwritev_datasieving (ompio_file_t *fh );
static ssize_t mca_fbtl_ucx_pwritev_generic (ompio_file_t *fh );

ssize_t  mca_fbtl_ucx_pwritev(ompio_file_t *fh )
{
    ...
    if ( fh->f_num_of_io_entries > 1 ) {
        opal_output (0, "Multiple entries, in pwritev \n");
        ...
        if ( false == mca_fbtl_ucx_write_datasieving      ||
             0     == avg_gap_size                          ||
             avg_block_size > mca_fbtl_ucx_max_block_size ||
             avg_gap_size   > mca_fbtl_ucx_max_gap_size   ||
        ...
        if ( do_data_sieving) {
            return mca_fbtl_ucx_pwritev_datasieving (fh);
        }
        else {
            return mca_fbtl_ucx_pwritev_generic (fh);
        }
    }
    else {
        // i.e. fh->f_num_of_io_entries == 1
        opal_output (0, "One entry, in pwritev \n"); 
        ret = mca_fbtl_ucx_lock ( &lock, fh, F_WRLCK, (off_t)fh->f_io_array[0].offset,
                                    (off_t)fh->f_io_array[0].length, OMPIO_LOCK_ENTIRE_REGION ); 
        if ( 0 < ret ) {
            opal_output(1, "mca_fbtl_ucx_pwritev: error in mca_fbtl_ucx_lock() ret=%d: %s",
                        ret, strerror(errno));
            /* Just in case some part of the lock worked */
            mca_fbtl_ucx_unlock ( &lock, fh);
            return OMPI_ERROR;
        }
        
        ret_code = ucx_bb_pwrite(fh->fd, fh->f_io_array[0].memory_address, fh->f_io_array[0].length,
                         (off_t)fh->f_io_array[0].offset, fh->f_filename, fh->f_rank);
        if ( ret_code == -1 ) {
            opal_output(1, "mca_fbtl_ucx_pwritev: error in ucx_bb_pwrite:%s", strerror(errno));
        }
        /* ret_code = pwrite(fh->fd, fh->f_io_array[0].memory_address, fh->f_io_array[0].length,
                         (off_t)fh->f_io_array[0].offset ); */
        mca_fbtl_ucx_unlock ( &lock, fh );
        /* if ( ret_code == -1 ) {
            opal_output(1, "mca_fbtl_ucx_pwritev: error in (p)write(v):%s", strerror(errno));
	    return OMPI_ERROR;
	} */
...
ssize_t mca_fbtl_ucx_pwritev_datasieving (ompio_file_t *fh)
{
    ...
    while (!done) {
        ...
        // buffer does not exceed mca_fbtl_ucx_max_tmpbuf_size bytes.
        ...
        for ( j = startindex; j < fh->f_num_of_io_entries; j++ ) {
            ...
            if (slen > mca_fbtl_ucx_max_tmpbuf_size ) {
                endindex = j-1;
                break;
            }
        }
        ...
        ret = mca_fbtl_ucx_lock ( &lock, fh, F_WRLCK, start, len, OMPIO_LOCK_ENTIRE_REGION ); 
        if ( 0 < ret ) {
            opal_output(1, "mca_fbtl_ucx_pwritev_datasieving: error in mca_fbtl_ucx_lock() ret=%d: %s",
                        ret, strerror(errno));
            /* Just in case some part of the lock worked */
            mca_fbtl_ucx_unlock ( &lock, fh);
            free ( temp_buf);
        ...
        if ( ret_code == -1 ) {
            //opal_output(1, "mca_fbtl_ucx_pwritev_datasieving: error in pwrite:%s", strerror(errno));
            opal_output(1, "mca_fbtl_ucx_pwritev_datasieving: error in pwrite:%s", strerror(errno));
            /* Just in case some part of the lock worked */
            mca_fbtl_ucx_unlock ( &lock, fh);
            free ( temp_buf);
...
ssize_t mca_fbtl_ucx_pwritev_generic (ompio_file_t *fh )
{
    /*int *fp = NULL;*/
    int i, block = 1, ret;
    ...
    opal_output (0, "inside pwritev generic \n"); 
	...
	/*
	  opal_output (0, "RANK: %d Entries: %d count: %d\n",
	  fh->f_rank,
	...
	*/
        ...
        ret = mca_fbtl_ucx_lock ( &lock, fh, F_WRLCK, iov_offset, total_length, OMPIO_LOCK_SELECTIVE ); 
        if ( 0 < ret ) {
            opal_output(1, "mca_fbtl_ucx_pwritev: error in mca_fbtl_ucx_lock() error ret=%d %s", ret, strerror(errno));
            free (iov); 
            /* just in case some part of the lock worked */
            mca_fbtl_ucx_unlock ( &lock, fh );
            return OMPI_ERROR;
    ...
    ret_code = ucx_bb_pwritev (fh->fd, iov, iov_count, iov_offset, fh->f_filename, total_length, fh->f_rank);
    if (-1 == ret_code ) {
	    opal_output(1, "mca_fbtl_ucx_pwritev: error in ucx_bb_pwritev:%s", strerror(errno));
            free (iov);
            return OMPI_ERROR;
	}
	...
	if (-1 == lseek (fh->fd, iov_offset, SEEK_SET)) {
	    opal_output(1, "mca_fbtl_ucx_pwritev: error in lseek:%s", strerror(errno));
            free(iov);
            mca_fbtl_ucx_unlock ( &lock, fh );
	    return OMPI_ERROR;
	}
	...
	if ( 0 < ret_code ) {
	    ...
	    opal_output(1, "mca_fbtl_ucx_pwritev: error in writev:%s", strerror(errno));
            free (iov);
            return OMPI_ERROR;
	}
	iov_count = 0;
