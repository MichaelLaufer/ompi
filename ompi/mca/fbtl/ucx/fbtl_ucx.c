...
int fbtl_ucx_max_aio_active_reqs=2048;

#include "ompi/mca/fbtl/fbtl.h"
#include "ompi/mca/fbtl/ucx/fbtl_ucx.h"
 ...
 * *******************************************************************
 */
static mca_fbtl_base_module_1_0_0_t ucx =  {
    mca_fbtl_ucx_module_init,     /* initalise after being selected */
    mca_fbtl_ucx_module_finalize, /* close a module on a communicator */
    mca_fbtl_ucx_preadv,          /* blocking read */
#if defined (FBTL_ucx_HAVE_AIO)
    mca_fbtl_ucx_ipreadv,         /* non-blocking read*/
#else
    NULL,                           /* non-blocking read */
#endif
    mca_fbtl_ucx_pwritev,         /* blocking write */
#if defined (FBTL_ucx_HAVE_AIO)
    mca_fbtl_ucx_ipwritev,        /* non-blocking write */
    mca_fbtl_ucx_progress,        /* module specific progress */
    mca_fbtl_ucx_request_free     /* free module specific data items on the request */
#else
...
int mca_fbtl_ucx_component_init_query(bool enable_progress_threads,
...
mca_fbtl_ucx_component_file_query (ompio_file_t *fh, int *priority) {
   *priority = mca_fbtl_ucx_priority;
...
int mca_fbtl_ucx_component_file_unquery (ompio_file_t *file) {
...
int mca_fbtl_ucx_module_init (ompio_file_t *file) {
   	//srand(time(NULL));
#if defined (FBTL_ucx_HAVE_AIO)
    long val = sysconf(_SC_AIO_MAX);
    if ( -1 != val ) {
	fbtl_ucx_max_aio_active_reqs = (int)val;
    }
#endif
    return OMPI_SUCCESS;
}
...
int mca_fbtl_ucx_module_finalize (ompio_file_t *file) {
    return OMPI_SUCCESS;
}

bool mca_fbtl_ucx_progress ( mca_ompio_request_t *req)
{
    bool ret=false;
#if defined (FBTL_ucx_HAVE_AIO)
    int i=0, lcount=0, ret_code=0;
    mca_fbtl_ucx_request_data_t *data=(mca_fbtl_ucx_request_data_t *)req->req_data;
    ...
    if ( (lcount == data->aio_req_chunks) && (0 != data->aio_open_reqs )) {
        ...
        mca_fbtl_ucx_unlock ( &data->aio_lock, data->aio_fh );
	...
	if ( (data->aio_req_count-data->aio_last_active_req) > data->aio_req_chunks ) {
        ...
        if ( FBTL_ucx_READ == data->aio_req_type ) {
            ret_code = mca_fbtl_ucx_lock( &data->aio_lock, data->aio_fh, F_RDLCK, start_offset, total_length, OMPIO_LOCK_ENTIRE_REGION );
        }
        else if ( FBTL_ucx_WRITE == data->aio_req_type ) {
            ret_code = mca_fbtl_ucx_lock( &data->aio_lock, data->aio_fh, F_WRLCK, start_offset, total_length, OMPIO_LOCK_ENTIRE_REGION );
        }
        if ( 0 < ret_code ) {
            opal_output(1, "mca_fbtl_ucx_progress: error in mca_fbtl_ucx_lock() %d", ret_code);
	...
	for ( i=data->aio_first_active_req; i< data->aio_last_active_req; i++ ) {
		...
		if (-1 == aio_read(&data->aio_reqs[i])) {
		    opal_output(1, "mca_fbtl_ucx_progress: error in aio_read()");
                    mca_fbtl_ucx_unlock ( &data->aio_lock, data->aio_fh );
		    return OMPI_ERROR;
		}
	    }
		...
		if (-1 == aio_write(&data->aio_reqs[i])) {
		    opal_output(1, "mca_fbtl_ucx_progress: error in aio_write()");
                    mca_fbtl_ucx_unlock ( &data->aio_lock, data->aio_fh );
		    return OMPI_ERROR;
		}
	    }
...
void mca_fbtl_ucx_request_free ( mca_ompio_request_t *req)
{
#if defined (FBTL_ucx_HAVE_AIO)
