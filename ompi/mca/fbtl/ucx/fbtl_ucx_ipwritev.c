...
ssize_t  mca_fbtl_ucx_ipwritev (ompio_file_t *fh,
				 ompi_request_t *request)
{
opal_output (0, "Inside ipwritev\n"); 

#if defined(FBTL_ucx_HAVE_AIO)
    mca_fbtl_ucx_request_data_t *data;
    mca_ompio_request_t *req = (mca_ompio_request_t *) request;
    int i=0, ret;
    off_t start_offset, end_offset, total_length;

    data = (mca_fbtl_ucx_request_data_t *) malloc ( sizeof (mca_fbtl_ucx_request_data_t));
    if ( NULL == data ) {
        opal_output (1,"mca_fbtl_ucx_ipwritev: could not allocate memory\n");
        return 0;
    }

    data->aio_req_count = fh->f_num_of_io_entries;
    data->aio_open_reqs = fh->f_num_of_io_entries;
    data->aio_req_type  = FBTL_ucx_WRITE;
    data->aio_req_chunks = fbtl_ucx_max_aio_active_reqs;
    ...
    ret = mca_fbtl_ucx_lock( &data->aio_lock, data->aio_fh, F_WRLCK, start_offset, total_length, OMPIO_LOCK_ENTIRE_REGION );
    if ( 0 < ret ) {
        opal_output(1, "mca_fbtl_ucx_ipwritev: error in mca_fbtl_ucx_lock() error ret=%d %s", ret, strerror(errno));
	...
	if ( MAX_ATTEMPTS == counter ) {
            opal_output(1, "mca_fbtl_ucx_ipwritev: error in aio_write():  %s", strerror(errno));
            mca_fbtl_ucx_unlock ( &data->aio_lock, data->aio_fh );                    
            free(data->aio_req_status);
            free(data->aio_reqs);
            free(data);
    ...
    req->req_data = data;
    req->req_progress_fn = mca_fbtl_ucx_progress;
    req->req_free_fn     = mca_fbtl_ucx_request_free;
#endif
    return OMPI_SUCCESS;
}
