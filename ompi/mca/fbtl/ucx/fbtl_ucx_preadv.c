...
static ssize_t mca_fbtl_ucx_preadv_datasieving (ompio_file_t *fh);
static ssize_t mca_fbtl_ucx_preadv_generic (ompio_file_t *fh);

ssize_t mca_fbtl_ucx_preadv (ompio_file_t *fh )
{
    ...
    if ( fh->f_num_of_io_entries > 1 ) {
        ...
        if ( false == mca_fbtl_ucx_read_datasieving       ||
             0     == avg_gap_size                          ||
        ...
        if ( do_data_sieving) {
            return mca_fbtl_ucx_preadv_datasieving (fh);
        }
        else {
            return mca_fbtl_ucx_preadv_generic (fh);
        }
    }
    else {
        // i.e. fh->f_num_of_io_entries == 1
        ret = mca_fbtl_ucx_lock ( &lock, fh, F_RDLCK, (off_t)fh->f_io_array[0].offset,
                                    (off_t)fh->f_io_array[0].length, OMPIO_LOCK_ENTIRE_REGION ); 
        if ( 0 < ret ) {
            opal_output(1, "mca_fbtl_ucx_preadv: error in mca_fbtl_ucx_lock() ret=%d: %s",
                        ret, strerror(errno));
        ...
        mca_fbtl_ucx_unlock ( &lock, fh );
        if ( ret_code == -1 ) {
            opal_output(1, "mca_fbtl_ucx_preadv: error in (p)read(v):%s", strerror(errno));
	    return OMPI_ERROR;
	}
...
ssize_t mca_fbtl_ucx_preadv_datasieving (ompio_file_t *fh)
{
    ...
    while (!done) {
        ...
        ret = mca_fbtl_ucx_lock ( &lock, fh, F_RDLCK, start, len, OMPIO_LOCK_ENTIRE_REGION ); 
        if ( 0 < ret ) {
            opal_output(1, "mca_fbtl_ucx_preadv_datasieving: error in mca_fbtl_ucx_lock() ret=%d: %s",
                        ret, strerror(errno));
            /* Just in case some part of the lock worked */
            mca_fbtl_ucx_unlock ( &lock, fh);
            free ( temp_buf);
        ...
        mca_fbtl_ucx_unlock ( &lock, fh);
        if ( ret_code == -1 ) {
            opal_output(1, "mca_fbtl_ucx_preadv_datasieving: error in (p)read(v):%s", strerror(errno));
            free ( temp_buf);
            return OMPI_ERROR;
        }
...
ssize_t mca_fbtl_ucx_preadv_generic (ompio_file_t *fh )
{
    ssize_t bytes_read=0, ret_code=0;
    ...
    for (i=0 ; i<fh->f_num_of_io_entries ; i++) {
        ...
        ret = mca_fbtl_ucx_lock ( &lock, fh, F_RDLCK, iov_offset, total_length, OMPIO_LOCK_SELECTIVE ); 
        if ( 0 < ret ) {
            opal_output(1, "mca_fbtl_ucx_preadv_generic: error in mca_fbtl_ucx_lock() ret=%d: %s", ret, strerror(errno));
            free (iov);
            /* Just in case some part of the lock worked */
            mca_fbtl_ucx_unlock ( &lock, fh);
            return OMPI_ERROR;
        ...
        if (-1 == lseek (fh->fd, iov_offset, SEEK_SET)) {
            opal_output(1, "mca_fbtl_ucx_preadv_generic: error in lseek:%s", strerror(errno));
            free(iov);
            mca_fbtl_ucx_unlock ( &lock, fh );
            return OMPI_ERROR;
        }
        ...
        if ( 0 < ret_code ) {
            ...
            opal_output(1, "mca_fbtl_ucx_preadv_generic: error in (p)readv:%s", strerror(errno));
            free(iov);
            return OMPI_ERROR;
        }
        else if ( 0 == ret_code ){
