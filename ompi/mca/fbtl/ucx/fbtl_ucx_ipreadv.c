...
ssize_t mca_fbtl_ucx_ipreadv (ompio_file_t *fh,
			       ompi_request_t *request)
{
    ...
    if ( NULL == data ) {
        opal_output (1,"mca_fbtl_ucx_ipreadv: could not allocate memory\n");
        return 0;
    }

    data->aio_req_count = fh->f_num_of_io_entries;
    data->aio_open_reqs = fh->f_num_of_io_entries;
    data->aio_req_type  = FBTL_ucx_READ;
    ...
    ret = mca_fbtl_ucx_lock( &data->aio_lock, data->aio_fh, F_RDLCK, start_offset, total_length, OMPIO_LOCK_ENTIRE_REGION );
    if ( 0 < ret ) {
        opal_output(1, "mca_fbtl_ucx_ipreadv: error in mca_fbtl_ucx_lock() error ret=%d  %s", ret, strerror(errno));
	...
	if ( MAX_ATTEMPTS == counter ) {
           opal_output(1, "mca_fbtl_ucx_ipreadv: error in aio_read(): errno %d %s", errno, strerror(errno));
	   mca_fbtl_ucx_unlock ( &data->aio_lock, data->aio_fh );            
	   free(data->aio_reqs);
	   free(data->aio_req_status);
	   free(data);
